...
def message(msg, txt_colour, bkgd_colour):
    ...
    text_box = txt.get_rect(center=(dimension_x / 2, dimension_y / 2))
...
def game_loop():
    ...
    food_x = round(random.randrange(20, dimension_x - 20) / 20) * 20
    food_y = round(random.randrange(20, dimension_y - 20) / 20) * 20
    ...
    while not quit_game:
        ...
        while game_over:
            screen.fill(white)
            message("The snake has died! Press 'Q' to Quit or 'A' to play "
                    "Again", black, white)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                instructions = "Are you sure you want to exit the game? " \
                               "X to Quit, SPACE to resume, R to reset"
        ...
        clock.tick(speed)  # sets the speed at which each iteration of the
        # game loop
